"""
Google Scholar Search and Summarization Routes
"""
from fastapi import APIRouter, HTTPException, Body
from typing import Optional
import logging
from ....services.google_scholar_service import fetch_google_scholar_papers
...
async def search_google_scholar_papers(
    ...
    try:
        ...
        logging.info(f"Searching Google Scholar for query: {query}, limit: {limit}")
        
        papers = fetch_google_scholar_papers(query.strip(), limit=limit)
        ...
        logging.error(f"Error searching Google Scholar: {str(e)}", exc_info=True)
        raise HTTPException(
            status_code=500,
            detail={
...
async def extract_pdf_content(
    ...
    """
    Extract full text content from a PDF (works with any source).
...
async def summarize_paper_endpoint(
    ...
    """
    Summarize a research paper from Google Scholar.
...
async def ask_question_endpoint(
    ...
    """
    Ask a question about a research paper from Google Scholar.
    ...
    try:
        ...
        title = request_data.get("title", "")
        source = request_data.get("source", "Google Scholar")
